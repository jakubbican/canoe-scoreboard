// LayoutManager.jsx
// Handles responsive layouts for different display types
 ...
 */
export function LayoutProvider({ children, initialDisplayType = 'horizontal' }) {
  ...
  const [displayType, setDisplayType] = useState(urlParams.type || initialDisplayType);
  console.log("Using display type:", displayType);
  
  // Set custom config if width and height are provided in URL
  const initialCustomConfig = urlParams.width && urlParams.height ? {
  ...
  useEffect(() => {
    if (!DISPLAY_CONFIGS[displayType] && displayType !== 'custom') {
      console.warn(`Invalid display type "${displayType}". Falling back to horizontal.`);
      setDisplayType('horizontal');
    }
  }, [displayType]);

  // Get the active configuration
  const config = customConfig || DISPLAY_CONFIGS[displayType] || DISPLAY_CONFIGS.horizontal;
