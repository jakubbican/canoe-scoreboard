// WebSocketClient.jsx
// A robust implementation that maintains a single WebSocket connection
...
// WebSocket manager to handle connection outside React lifecycle
  ...
  connect(url) {
    ...
    try {
      ...
      globalWebSocket.onclose = (event) => {
        ...
        connectionListeners.forEach(listener => listener(false, 'Connection closed'));
 ...
 */
export function WebSocketProvider({ children, serverUrl = "ws://localhost:8081/" }) {
  // Connection state
  const [connected, setConnected] = useState(WebSocketManager.isConnected());
  const [connectionError, setConnectionError] = useState(null);
  const [lastMessageTime, setLastMessageTime] = useState(WebSocketManager.getLastMessageTime());
  ...
  // Initialize one WebSocket connection on page load or server URL change
  useEffect(() => {
    ...
    const removeConnectionListener = WebSocketManager.addConnectionListener((isConnected, error) => {
  ...
  // On component unmount, trigger disconnect
  useEffect(() => {
